...
public class InscriptionToTouristOuting extends JInternalFrame {
	...
	public InscriptionToTouristOuting(ITouristOutingAndInscriptionController itoic, ITouristActivityController itac,
		 ...
		 * gbc_lblInfoTouristOuting.gridx = 0; gbc_lblInfoTouristOuting.gridy = 2;
	...
	protected void loadTouristOutings() {
		...
		if (touristActivityName != null && !touristActivityName.isEmpty()) {
			...
			if (activityWithOutings != null) {
				...
				// Tengo un set de dtTouristOuting, me quiero quedar con una lista de nombres de
				...
				try {
					model = new DefaultComboBoxModel<String>(
							controlTouristOutingAndInscription.listTouristOutingsNames(allTouristOutings));
					comboBoxTouristOutings.setModel(model);
				} catch (TouristOutingDoesNotExistException e) {
					// We will not show any tourist outing
					// model = new DefaultComboBoxModel<String>(new String[] {"No existen salidas
	...
	private void showOutingDetails() {
		...
		if (touristOutingName != null && !touristOutingName.isEmpty()) {
			// Tengo el nombre de la salida que quiero, debo mostrar los datos de la misma
	...
	public void loadTourists() {
		...
		// } catch (ActivityDoesNotExistException e) {
		// We will not show any tourist activity
		// model = new DefaultComboBoxModel<String>(new String[] {"No existen turistas
		// registrados."});
		// }
	...
	protected void cmdInscriptionToTouristOutingActionPerformed(ActionEvent arg0) {
		// capta datos de inscripcion ingresados por el admin para dar de alta la
		// inscripcion
		// control chequear que elija usuario y que ingrese todos los datos de la
		// inscripcion
		String touristActivityName = (String) comboBoxTouristActivities.getSelectedItem();
		String touristOutingName = (String) comboBoxTouristOutings.getSelectedItem();
	...
	private boolean checkFormulario() {
		...
		if (touristActivityName == null || touristActivityName.isEmpty() || touristOutingName == null
				|| touristOutingName.isEmpty() || touristName == null || touristName.isEmpty()
				|| numTourists.isEmpty()) {
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "El numero de turistas debe ser un numero",
	...
	private void clearForm() {
		comboBoxTouristActivities.setSelectedIndex(-1);
		comboBoxTourists.setSelectedIndex(-1);
		comboBoxTouristOutings.setSelectedIndex(-1);
